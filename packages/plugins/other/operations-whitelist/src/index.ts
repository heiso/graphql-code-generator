...
import { concatAST, visit, print, OperationDefinitionNode, FragmentDefinitionNode, InlineFragmentNode } from 'graphql';
import {
...
export const plugin: PluginFunction<WhitelistPluginConfig, string> = (schema, documents, config) => {
  config = { ...defaultConfig, ...config };

  const documentNodes = documents.filter(document => !!document.document).map(document => document.document);
  ...
  documentNodes.forEach(documentNode => {
    visit(config.generateTypenames ? addTypenameToDocument(documentNode) : documentNode, {
      OperationDefinition: {
        enter(node) {
          if (!node.name) {
            throw new Error(`All graphql operation definitions should have a unique name: \n ${print(node)}`);
          }
...
export const validate: PluginValidateFn<WhitelistPluginConfig> = (schema, documents, config, outputFile) => {
  if (!config.output) {
